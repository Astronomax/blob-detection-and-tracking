#ifndef BLOB_TRACKER_KDTREE_HPP
#define BLOB_TRACKER_KDTREE_HPP
...
namespace kdt {
	...
	public:
		...
		void
		build(const std::vector<PointT>& points)
		{
			clear();
		...
		int
		nnSearch(const PointT& query, double* minDist = nullptr) const
		{
			int guess;
		...
		std::vector<int>
		knnSearch(const PointT& query, int k) const
		{
			KnnQueue queue(k);
		...
		std::vector<int>
		radiusSearch(const PointT& query, double radius) const
		{
	...
	private:
		...
		Node*
		buildRecursive(int* indices, int npoints, int depth)
		{
			if (npoints <= 0)
				return nullptr;
		...
		void
		validateRecursive(const Node* node, int depth) const
		{
			if (node == nullptr)
				return;
		...
		static double
		distance(const PointT& p, const PointT& q)
		{
			double dist = 0;
		...
		void
		nnSearchRecursive(const PointT& query, const Node* node, int *guess, double *minDist) const
		{
			if (node == nullptr)
				return;
		...
		void
		knnSearchRecursive(const PointT& query, const Node* node, KnnQueue& queue, int k) const
		{
			if (node == nullptr)
				return;
